...
for (sheet in sheet_names) {
  ...
  matched <- rownames(sample_data) %in% sheet_df$`Ensembl gene ID`
  sample_data$label[matched] <- sheet
}
```

```{r}
sample_data$label[is.na(sample_data$label)] <- "Healthy"
sample_data_clean <- sample_data[sample_data$label %in% c("CRC", "PAAD", "HBC"), ]

features <- sample_data_clean[, !(colnames(sample_data_clean) %in% "label")]
labels <- as.factor(sample_data_clean$label)
